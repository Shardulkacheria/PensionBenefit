...
export async function POST(request) {
  try {
    ...
    // Determine country/origin based on citizenship and state
    let origin = 'India'; // Default
    if (citizenship === 'No') {
      // You can add logic here to determine other countries based on state or other criteria
      origin = 'India'; // For now, keeping as India
    }
    ...
    if (currentPensionScheme && currentMonthlyPension) {
      ...
      if (currentScheme) {
        ...
        const recommendedPensionAmount = currentScheme.monthlyPension || 0;
    ...
    const topRecommendations = schemesWithPensions.slice(0, 3).map((scheme, index) => ({
      ...
      monthlyPension: scheme.monthlyPension,
